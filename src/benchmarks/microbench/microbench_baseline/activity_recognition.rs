...
use crate::{
    ...
    benchmarks::{
        ...
        print_current_task_stats, set_benchmark_done,
    },
...
fn train(model: &mut PBox<Model>, which: Class) {
    nv_for_loop!(ITER_TRAIN, i, 0 => MODEL_SIZE , {
        transaction::run_sys(|j, t| {
            ...
            let m = model.as_mut(j);
            match which {
                Class::ClassMoving => {
                    m.moving_features[i] = feature;
                },

                Class::ClassStationary => {
                    m.stationary_features[i] = feature;
                }
            }
        });
...
fn print_stats(stats: &PBox<Stats>) {
    let s = unsafe { stats.as_ref_no_journal() };
    bench_dbg_print!(
...
fn run_ar() {
    let (mut model, mut stats) = transaction::run_pure_sys(|t| {
        let mdl = PBox::new(
            Model {
        ...
        let stats = PBox::new(Stats::new(), t);
        (mdl, stats)
    });
    ...
    nv_for_loop!(STAGE, cnt, 0 => 4, {
        ...
        match mode {
            RunMode::ModeTrainStationary => {
                train(&mut model, Class::ClassStationary);
            },
            RunMode::ModeTrainMoving => {
                train(&mut model, Class::ClassMoving);
            },
            RunMode::ModeRecognize => {
                recognize(&mut model, &mut stats);
            },
            RunMode::ModeIdle => {
