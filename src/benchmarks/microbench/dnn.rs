...
pub fn fc_layer_impl2<const FC_H: usize, const FC_W: usize>(
    param: &Tensor2D<FC_H, FC_W>,
    input: &mut Ptr<Tensor1D<FC_W>>,
    output_ref: PRef<'_, Tensor1D<FC_H>>,
) {
    let param_h = FC_H;
    let param_w = FC_W;
    nv_for_loop!(H_CNT_2, i, 0 => param_h, {
        let input_ref = input.as_pref();
        transaction::run(|j| {
            let readable_ref = input_ref.into_readable();
            let input_ref = readable_ref.as_ref(j);
            let mut sum_i = 0;
            for j in 0..param_w {
...
fn dnn_inference() {
    ...
    let (mut ob1, mut ob2) = transaction::run_pure_sys(|t| {
        let output_buffer_1 = Ptr::new(Tensor1D::new([0; 10]), t);
        let output_buffer_2 = Ptr::new(Tensor1D::new([0; 2]), t);
    ...
    let result_buf = ob2.as_pref();
    transaction::run(|j| {
        let res = result_buf.into_readable();
        let res = res.as_ref(j);
