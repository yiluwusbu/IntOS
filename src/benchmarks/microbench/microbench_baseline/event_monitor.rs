...
fn collect_sensor_1_data(db: &mut DataBuffer, j: JournalHandle, t: SyscallToken) {
    ...
    for i in 0..DATA_BUF_SZ {
        db.data[i] = buffer.data[i];
    }
}

fn collect_sensor_2_data(db: &mut DataBuffer, j: JournalHandle, t: SyscallToken) {
    let ts = syscalls::sys_get_time(t);
    let ts = (ts % 128) as u8;
    let mut buffer = DataBuffer {
        data: [0; DATA_BUF_SZ],
...
fn task_actor_1(event_group: EventGroupHandle) {
    benchmark_start();
    let db = transaction::run_pure_sys(|t| {
        let db = PBox::new(
            DataBuffer {
                data: [0; DATA_BUF_SZ],
            },
...
fn monitor_task() {
    ...
    nv_for_loop!(BENCH_ITER_CNT,  i,  0 => EVENT_COUNT, {
        // #[cfg(not(feature="power_failure"))]
        // syscalls::sys_task_delay(EVENT_PERIOD);
        // event detected, notify other tasks
        // generate_event();
    ...
    // {
    //     transaction::run_pure_sys(|t| {
    //         syscalls::sys_task_delay_in_tx(40, t);
    //     })
    // }
    syscalls::sys_yield();
...
pub fn register() {
    task::register_app_no_param("event monitor", 1, monitor_task);
}
