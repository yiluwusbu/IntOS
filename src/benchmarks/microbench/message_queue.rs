...
fn task_entry() {
    ...
    transaction::run_pure_sys(|t| {
        ...
        syscalls::sys_create_task("net_agent", 1, task_network_agent, mq_box, t).unwrap();
...
fn task_network_agent(mq: PBox<MessageQueueGroup<Message>>) {
    ...
    task_yield();
    set_benchmark_done();
    bench_println!("Wallclk: {}", benchmark_clock());
...
fn task_worker_1(q: QueueHandle<Message>) {
    ...
    let (mut msg_buf, mut data_buf, mut pcnt) = transaction::run_pure_sys(|t| {
        let msg_buf = Ptr::new(msg, t);
        let data_buf = Ptr::new(DataBuffer { window: [0; 10] }, t);
        let pcnt = Ptr::new(0usize, t);
        (msg_buf, data_buf, pcnt)
    });

    loop {
        let pref = msg_buf.as_pref();
        let data_pref = data_buf.as_pref();
        let cnt_pref = pcnt.as_pref();
        transaction::run_sys_once(|j, t| {
            let r = syscalls::sys_queue_receive(q, 5000, t);
            match r {
                Ok(data) => {
                    // process data
                    let pref_rw = pref.write(data, j);
                    let msg = pref_rw.as_ref();
                    let tmp = msg.data;
                    let (i, cnt_pref_wlog) = cnt_pref.read(|v| *v, j);
                    data_pref.partial_write(|data_ref| &data_ref.window[i], tmp, j);
                    cnt_pref_wlog.write(|v| *v = (i + 1) % 10, j);
                    // bench_dbg_print!("i is: {}", i);
                    if i == 9 {
                        let (window, _) = data_pref.read(|r| r.window, j);
                        let mut sum = 0;
                        for i in 0..10 {
                            sum += window[i];
                        }
